...
function SessionCard({ session, onSelect, onDelete }: SessionCardProps) {
  ...
  const handleDelete = () => {
    Alert.alert(
      'Delete Session',
      'Are you sure you want to delete this reasoning session?',
      [
      ...
      {isExpanded && (
        <Animated.View style={expandedStyle} className="px-4 pb-4">
          ...
          <View className="mt-3 bg-green-50 border border-green-200 rounded-lg p-3">
            <Text className="text-xs font-medium text-green-700 mb-1">
              Solution:
            </Text>
...
export function ReasoningHistory() {
  ...
  const handleSessionSelect = (session: ReasoningSession) => {
    ...
    Alert.alert(
      'Session Loaded',
      'The reasoning session has been loaded into the main interface.',
        ...
        <View className="flex-row items-center justify-between">
          ...
          {sessions.length > 0 && (
            <View className="bg-blue-50 px-3 py-1 rounded-full">
              <Text className="text-xs font-medium text-blue-700">
                Last: {sessions[0]?.tokensPerSecond || 0} t/s
              </Text>
            </View>
          )}
        </View>
                ...
                <View className="flex-row justify-between items-center mb-2">
                  ...
                  <Text className="text-sm font-medium text-green-600">
                    {(
                      ...
                      sessions.filter(s => s.duration).length / 1000
                    ).toFixed(1)}s
                  </Text>
                </View>
