...
export function ReasoningInterface() {
  ...
  const progressBarStyle = useAnimatedStyle(() => {
    const backgroundColor = interpolateColor(progressValue.value, [0, 1], ["#3B82F6", "#10B981"]);
    return {
      backgroundColor,
  ...
  const handleGenerate = async () => {
    ...
    // Button animation
    generateButtonScale.value = withSequence(withSpring(0.95), withSpring(1));
  ...
  return (
    ...
    >
      ...
      <View className="bg-white border-b border-gray-200 px-4 py-3">
        <View className="flex-row items-center justify-between">
          ...
          {lastTokensPerSecond > 0 && (
            <View className="bg-blue-50 px-3 py-1 rounded-full">
              <Text className="text-xs font-medium text-blue-700">{lastTokensPerSecond} tokens/sec</Text>
            </View>
          )}
        </View>
        ...
        {isGenerating && (
          <View className="mt-3 h-1 bg-gray-200 rounded-full overflow-hidden">
            <Animated.View className="h-full rounded-full" style={progressBarStyle} />
          </View>
        )}
      </View>
      ...
      <ScrollView ref={scrollViewRef} className="flex-1 px-4" showsVerticalScrollIndicator={false}>
        {/* Input Section */}
        <View className="py-4">
          <Text className="text-lg font-semibold text-gray-900 mb-3">Mathematical Problem</Text>
          ...
          <TextInput
            ...
            accessibilityHint={
              errorMessage || "Enter a mathematical problem such as an equation, geometry question, or word problem"
            }
            accessibilityState={{ disabled: !!errorMessage }}
          />
          ...
          {/* Example Problems */}
          <View className="mt-4">
            <Text className="text-sm font-medium text-gray-700 mb-2">Example Problems:</Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              <View className="flex-row space-x-2">
                {EXAMPLE_PROBLEMS.map((example, index) => (
                  ...
                  >
                    <Text className="text-xs font-medium text-blue-700 mb-1">{example.category}</Text>
          ...
          <View className="flex-row space-x-3 mt-4">
            <AnimatedPressable
              ...
              accessibilityLabel={isGenerating ? "Generating reasoning" : "Generate reasoning"}
              accessibilityHint={
                isGenerating
                  ? "Please wait while the AI solves your problem"
                  : !!inputText.trim()
                    ? "Tap to start solving the mathematical problem"
                    : "Enter a problem first, then tap to generate reasoning"
              }
              accessibilityState={{ disabled: !inputText.trim() || isGenerating, busy: isGenerating }}
            >
              <Ionicons
                ...
                style={{ marginRight: 8 }}
              />
              <Text className="text-white font-semibold text-base">{isGenerating ? "Reasoning..." : "Generate"}</Text>
            </AnimatedPressable>
        ...
        {(currentProblem || isGenerating) && (
          <View className="bg-white rounded-xl p-4 mb-4">
            {/* Problem Statement */}
            <View className="mb-4">
              <Text className="text-sm font-medium text-gray-500 mb-1">PROBLEM:</Text>
              <Text className="text-base text-gray-900">{currentProblem}</Text>
            </View>

            {/* Reasoning Steps */}
            {currentSteps.length > 0 && (
              <View className="mb-4">
                <Text className="text-sm font-medium text-gray-500 mb-3">REASONING STEPS:</Text>
                {currentSteps.map((step, index) => (
                  <View key={index} className="mb-3 border-l-4 border-blue-200 pl-3">
                    ...
                    <Text className="text-sm text-gray-700">{step.content}</Text>
                  </View>
                ))}
              </View>
            )}
            ...
            {(currentSolution || streamedSolution) && (
              <View className="bg-green-50 border border-green-200 rounded-lg p-3">
                <Text className="text-sm font-medium text-green-700 mb-1">SOLUTION:</Text>
                <Text className="text-base text-green-900">{currentSolution || streamedSolution}</Text>

                {/* Performance Metrics */}
                {lastDuration > 0 && (
                  <View className="flex-row justify-between mt-3 pt-2 border-t border-green-200">
                    <Text className="text-xs text-green-600">Duration: {(lastDuration / 1000).toFixed(1)}s</Text>
                    <Text className="text-xs text-green-600">Speed: {lastTokensPerSecond} t/s</Text>
                  </View>
                )}
              </View>
            )}
        ...
        <Animated.View style={loadingStyle} className="items-center py-4">
          <View className="bg-white rounded-xl p-6 items-center">
            ...
            <Text className="text-base font-medium text-gray-900 mt-2">Phi-4 is thinking...</Text>
            <Text className="text-sm text-gray-500 mt-1">Running on Apple Neural Engine</Text>
          </View>
        </Animated.View>
      </ScrollView>
    </KeyboardAvoidingView>
