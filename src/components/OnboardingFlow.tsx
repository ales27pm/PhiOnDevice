...
export function OnboardingFlow({ onComplete }: OnboardingFlowProps) {
  ...
  const handleNext = () => {
    if (currentStep < ONBOARDING_STEPS.length - 1) {
      Analytics.trackUserInteraction('next_step', 'onboarding', currentStep + 1);
      
      slideAnimation.value = withSpring(1, { duration: 300 }, () => {
  ...
  const handlePrevious = () => {
    if (currentStep > 0) {
      Analytics.trackUserInteraction('previous_step', 'onboarding', currentStep - 1);
  ...
  const handleSkip = () => {
    Analytics.trackUserInteraction('skip_onboarding', 'onboarding', currentStep);
    handleComplete();
  };
  ...
  const handleComplete = () => {
    ...
    fadeAnimation.value = withSpring(0, { duration: 400 }, () => {
      runOnJS(onComplete)();
    });
  };

  const goToStep = (stepIndex: number) => {
    Analytics.trackUserInteraction('jump_to_step', 'onboarding', stepIndex);
      ...
      <Animated.View style={containerStyle} className="flex-1">
        ...
        <View className="px-6 py-4 flex-row items-center justify-between">
          ...
          <Pressable
            ...
            {...AccessibilityHelper.getButtonProps('Skip onboarding', 'Skip the introduction and go directly to the app')}
          >
          ...
          <View className="flex-row space-x-2">
            {ONBOARDING_STEPS.map((_, index) => (
              <Pressable
                ...
                className="flex-1"
                {...AccessibilityHelper.getButtonProps(
                  `Go to step ${index + 1}`,
                  `Jump to step ${index + 1}: ${ONBOARDING_STEPS[index].title}`
                )}
              >
                <View 
                  className={cn(
        ...
        >
          ...
          <View className="mb-8">
            <Text 
              ...
              {...AccessibilityHelper.getHeaderProps(1, currentStepData.title)}
            >
          ...
          {currentStepData.features && (
            <View className="bg-white rounded-xl p-6 mb-8">
              <Text className="text-lg font-semibold text-gray-900 mb-4">
                {currentStep === 0 ? 'Key Features' : 
                 currentStep === 1 ? 'Problem Types' :
                 currentStep === 2 ? 'Reasoning Steps' :
                 currentStep === 3 ? 'Technical Specs' : 'Privacy Benefits'}
              </Text>
              ...
              {currentStepData.features.map((feature, index) => (
                <View 
                  ...
                  {...AccessibilityHelper.getListItemProps(feature, index + 1, currentStepData.features!.length)}
                >
        ...
        <View className="px-6 py-4 flex-row items-center justify-between" style={{ paddingBottom: insets.bottom + 16 }}>
          <Pressable
            ...
            {...AccessibilityHelper.getButtonProps(
              'Previous step',
              currentStep === 0 ? 'No previous step available' : 'Go to previous step',
              currentStep === 0
            )}
          >
            <Ionicons 
          ...
          <Pressable
            ...
            {...AccessibilityHelper.getButtonProps(
              ...
              currentStep === ONBOARDING_STEPS.length - 1 ? 'Complete onboarding and start using the app' : 'Continue to next step'
            )}
          >
